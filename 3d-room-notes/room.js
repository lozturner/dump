...
makeWall(ROOM.w, ROOM.h, new THREE.Vector3(0, ROOM.h/2, -ROOM.d/2), 0);                    // back
makeWall(ROOM.w, ROOM.h, new THREE.Vector3(0, ROOM.h/2,  ROOM.d/2), Math.PI);              // front
makeWall(ROOM.d, ROOM.h, new THREE.Vector3(-ROOM.w/2, ROOM.h/2, 0), Math.PI/2);            // left
makeWall(ROOM.d, ROOM.h, new THREE.Vector3( ROOM.w/2, ROOM.h/2, 0), -Math.PI/2);           // right
...
const bedFrame = new THREE.Mesh(
  new THREE.BoxGeometry(1.6, 0.35, 2.2),
  new THREE.MeshStandardMaterial({ color: 0x3a2a1f, roughness: 0.8 })
);
...
const mattress = new THREE.Mesh(
  new THREE.BoxGeometry(1.5, 0.22, 2.05),
  new THREE.MeshStandardMaterial({ color: 0xf1eadb, roughness: 0.9 })
);
...
const duvet = new THREE.Mesh(
  new THREE.BoxGeometry(1.52, 0.12, 1.4),
...
pillow.position.set(0, 0.64, -0.75);
pillow.castShadow = true;
bedGroup.add(pillow);

const headboard = new THREE.Mesh(
  new THREE.BoxGeometry(1.7, 0.9, 0.1),
  new THREE.MeshStandardMaterial({ color: 0x2a1e15, roughness: 0.7 })
);
headboard.position.set(0, 0.7, -1.12);
...
const cabinet = addBox(1.2, 1.4, 0.5, 2.5, 0.7, -ROOM.d/2 + 0.28, 0x6b4a30);
// drawers
for (let i = 0; i < 3; i++) {
  const drawer = new THREE.Mesh(
    new THREE.BoxGeometry(1.05, 0.36, 0.02),
  ...
  const handle = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xd1a15a, metalness: 0.8, roughness: 0.3 })
  );
...
const pot = new THREE.Mesh(
  new THREE.CylinderGeometry(0.12, 0.09, 0.18, 20),
  new THREE.MeshStandardMaterial({ color: 0x8a5a3a, roughness: 0.8 })
);
pot.position.set(2.5, 1.49, -ROOM.d/2 + 0.28);
pot.castShadow = true;
scene.add(pot);
const plant = new THREE.Mesh(
  new THREE.IcosahedronGeometry(0.22, 0),
  new THREE.MeshStandardMaterial({ color: 0x3f7a3a, roughness: 0.95, flatShading: true })
);
plant.position.set(2.5, 1.75, -ROOM.d/2 + 0.28);
...
const rug = new THREE.Mesh(
  ...
  new THREE.MeshStandardMaterial({ color: 0x7a2e2e, roughness: 0.95 })
);
rug.rotation.x = -Math.PI/2;
rug.position.set(0.2, 0.005, 1.0);
...
lampBase.position.set(-3.5, 0.025, 3);
scene.add(lampBase);
const lampPole = new THREE.Mesh(
  new THREE.CylinderGeometry(0.02, 0.02, 1.6, 8),
  new THREE.MeshStandardMaterial({ color: 0x222, metalness: 0.5 })
);
lampPole.position.set(-3.5, 0.85, 3);
scene.add(lampPole);
const lampShade = new THREE.Mesh(
  new THREE.ConeGeometry(0.25, 0.3, 16, 1, true),
  new THREE.MeshStandardMaterial({ color: 0xf4b860, side: THREE.DoubleSide, emissive: 0x8a4a10, emissiveIntensity:0.4 })
);
lampShade.position.set(-3.5, 1.7, 3);
scene.add(lampShade);
const lampLight = new THREE.PointLight(0xffc98a, 0.8, 5, 1.8);
lampLight.position.set(-3.5, 1.55, 3);
scene.add(lampLight);
...
// on touch we don't use pointerlock; we simulate "locked" with a flag
...
controls.addEventListener('unlock', () => {
  if (!noteModal.classList.contains('on') && !boardModal.classList.contains('on')) {
...
(function setupLook() {
  ...
  const pitchObj = camera; // PointerLockControls rotates yaw on obj, pitch on camera
...
function placeNote() {
  ...
  // build canvas texture for the note
...
// mirror main board into modal canvas
function syncFromBoard() {
  bigCtx.drawImage(boardCanvas, 0, 0);
}
function syncToBoard() {
  bctx.drawImage(bigCanvas, 0, 0);
...
function boardPt(e) {
  ...
  return {
    x: (clientX - r.left) * (bigCanvas.width / r.width),
    y: (clientY - r.top)  * (bigCanvas.height / r.height)
  };
}
...
function animate() {
  ...
  if (isActive()) {
    ...
    // keep notes facing player (billboard)
    for (const n of notes) {
      n.lookAt(camera.position);
    }
  }

  // sync avatar to player each frame so top view shows them correctly
